...
| `GET` | `/doc/library/{doc_id}/source` | The stored original file, served as the media type it was admitted as |
| `GET` | `/doc/library/{doc_id}/source/info` | Original-file metadata (no bytes); `offset`/`limit` read a line window of a text original |
...
- `POST /doc/parse` accepts `store_only=true` for the **raw channel**: `md` and `png/jpg/jpeg/gif/webp` are stored as the original file with no parse products at all, and read back via `/doc/library/{doc_id}/source`. The response and manifest carry `kind: "raw"` (parsed documents read `"parsed"`), as do search hits, the sections listing and summary status; such documents 404 on digest/brief/full, 409 on a summary retry, and do not appear in `search_text`. `GET /doc/health` lists the channel's formats as `raw_formats`. A `409` on this channel returns an object under `detail` carrying the existing document's `kind`; the parse channel's `409` is unchanged. The raw and parsed format sets never overlap — a parseable extension asking for `store_only` is a `422`. Requires `MANTISFETCH_STORE_SOURCE_FILES=true`.
...
| `MANTISFETCH_SEARCH_MAX_WAIT_SEC` | `30` | How long a search may wait for its turn before `429` instead. A burst of `max_wait / interval` searches is absorbed, plus the one that does not wait at all — 16 at the defaults; `0` = never wait |
...
| `MANTISFETCH_PARSE_QUEUE_MAX_BYTES` | ten times the upload cap (**2GB** at the default 200MB) | Staged upload bytes the parse queue may hold. A queued parse keeps its upload on disk, so the queue is bounded by disk rather than by count. The same budget also covers uploads still being received: a `/parse` request is admitted by its `Content-Length` (the largest request size if it declares none) against what is queued plus what is arriving, before any of its body is read, and gets `429` if it does not fit — so this one number bounds all upload bytes the service holds |
| `MANTISFETCH_MAX_PARSE_ROWS` | `100000` | Rows an XLSX may have across all its sheets. Counted by streaming the sheet XML before conversion; a workbook over it gets `422` and nothing is stored. It is a memory bound: conversion holds the whole workbook, and 100k rows × 10 columns measured about 2GB RSS. Very wide sheets are bounded by the OOXML unzip budget instead, which now covers XLSX and PPTX as well as DOCX. `.xls` is not counted |
...
| `GET` | `/doc/library/{doc_id}/source` | 入库原件字节，按入库时记录的 media type 返回 |
| `GET` | `/doc/library/{doc_id}/source/info` | 原件元数据（不返字节）；带 `offset`/`limit` 时返回文本原件的行窗口 |
...
- `POST /doc/parse` 支持 `store_only=true` 走**原件通道**：`md` 与 `png/jpg/jpeg/gif/webp` 只存原件、完全不解析，通过 `/doc/library/{doc_id}/source` 读回。响应与 manifest 带 `kind: "raw"`（解析文档为 `"parsed"`），搜索命中、sections 列举、摘要状态同样带；这类文档在 digest/brief/full 上 404、摘要重试上 409，也不会出现在 `search_text` 结果里。`GET /doc/health` 以 `raw_formats` 列出本通道的格式。本通道的 `409` 在 `detail` 里返回对象、带上已有文档的 `kind`；解析通道的 `409` 不变。原件与解析两套扩展名集合互不相交 —— 可解析的扩展名带 `store_only` 会返回 `422`。要求 `MANTISFETCH_STORE_SOURCE_FILES=true`。
...
| `MANTISFETCH_SEARCH_MAX_WAIT_SEC` | `30` | 一次搜索最多等多久轮到自己，超过才 `429`。能吸收 `max_wait / interval` 个排队的，再加上不用等的那一个——默认值下是 16；`0` = 从不等待 |
...
| `MANTISFETCH_PARSE_QUEUE_MAX_BYTES` | 上传上限的十倍（默认 200MB 时为 **2GB**） | 解析队列允许占用的暂存上传字节数。排队中的解析会一直把上传留在盘上，所以队列按磁盘设限而不是按个数。同一额度也覆盖「正在接收中」的上传：`/parse` 请求在读取任何请求体之前，按 `Content-Length`（未声明长度的按单请求上限计）与「已排队 + 正在接收」的字节合计准入，放不下就 `429`——所以这一个数限定了服务持有的全部上传字节 |
| `MANTISFETCH_MAX_PARSE_ROWS` | `100000` | XLSX 所有 sheet 合计允许的行数。转换前流式读 sheet XML 计数，超过即 `422`，什么都不入库。这是内存上限：转换会把整个工作簿读进内存，实测 10 万行 × 10 列约 2GB RSS。特别宽的表由 OOXML 解压预算兜底，该预算现已同时覆盖 XLSX、PPTX 与 DOCX。`.xls` 不计数 |
