...
| `MANTISFETCH_PARSE_QUEUE_MAX_BYTES` | ten times the upload cap (**2GB** at the default 200MB) | Staged upload bytes the parse queue may hold. A queued parse keeps its upload on disk, so the queue is bounded by disk rather than by count. The same number separately bounds uploads still being received: a `/parse` request is admitted by its `Content-Length` (the largest request size if it declares none) before any of its body is read, and gets `429` if it does not fit |
...
| `MANTISFETCH_PARSE_QUEUE_MAX_BYTES` | 上传上限的十倍（默认 200MB 时为 **2GB**） | 解析队列允许占用的暂存上传字节数。排队中的解析会一直把上传留在盘上，所以队列按磁盘设限而不是按个数。同一个数还单独限制「正在接收中」的上传：`/parse` 请求在读取任何请求体之前按 `Content-Length` 准入（未声明长度的按单请求上限计），放不下就 `429` |
