...
> 没有 `fetch_and_parse`/`get_document`/`get_sections_batch`/`get_cached`/`list_tables`/`search_library`
> 这些工具——写 SKILL/system prompt 时用上表的真实名字。批量取章节、URL 缓存等见 §10（未实现）。
...
> 注意：当前没有"一次调用内联前 N token 正文 + 截断标志"这类参数（`max_tokens`/`inline_content`），
> 也没有 `get_sections_batch` 批量接口——见 §10。
...
| `get_sections_batch` 批量取章节 | 未实现 | 跨主机往返延迟敏感，值得做；现状逐个 `doc_section` |
