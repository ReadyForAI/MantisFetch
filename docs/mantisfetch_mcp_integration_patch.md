...
>
> **数据通道补充**：三级加载解决的是"LLM 需要阅读分析"的场景。"工具代码需要读数据但 LLM 不需要理解"的场景（格式检查、字段提取、图片读取、上传发布）由 SKILL 脚本用 `MantisFetchClient` 直接调 HTTP REST API，内容不经过 LLM、零 token。MantisFetch 即共享文件系统——不要把文档下载到本地，每次读实时走 HTTP（章节 `GET /doc/library/{id}/section/{sid}`、列表 `/sections`、图片元数据+OCR `GET /doc/library/{id}/image/{id}`、图片**原始字节**（视觉读图/签章识别）`GET /doc/library/{id}/image/{id}/raw`）。对应 `MantisFetchClient` 方法：`get_section`/`list_sections`/`get_sections_batch`/`get_table`/`get_table_json`/`list_images`/`get_image`/`get_image_bytes`/`get_summary`（sync + async 均有）。
...
Agent 生成的文档（投标文件、分析报告等）先写本地 workspace 作草稿，最终版用 `MantisFetchClient.parse()` 上传到 MantisFetch 共享档案库——走 HTTP REST（`POST /doc/parse` multipart），不走 MCP、不消耗 token，LLM 只看到返回的 `doc_id`。发布后拿 `doc_id`，后续流程（审查、交叉引用、归档）按此 id 读取，操作方式和用户上传的文档完全一致——消费方不区分来源。典型：bid-agent 生成投标文件 → 上传发布 → review-agent 拿 doc_id 做标书审查。review-agent 也可用 `client.search(..., metadata={"stage": "pending-review"})` 找出待审文档。
