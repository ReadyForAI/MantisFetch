...
>
> **数据通道补充**：三级加载解决的是"LLM 需要阅读分析"的场景。"工具代码需要读数据但 LLM 不需要理解"的场景（格式检查、字段提取、图片读取、上传发布）由 SKILL 脚本用 `MantisFetchClient` 直接调 HTTP REST API，内容不经过 LLM、零 token。MantisFetch 即共享文件系统——不要把文档下载到本地，每次读实时走 HTTP（章节 `GET /doc/library/{id}/section/{sid}`、列表 `/sections`、图片元数据+OCR `GET /doc/library/{id}/image/{id}`、图片**原始字节**（视觉读图/签章识别）`GET /doc/library/{id}/image/{id}/raw`）。
