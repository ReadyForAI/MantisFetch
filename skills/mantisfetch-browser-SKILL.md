...
- **Targeting a search provider (`provider` on `web_search` / `web_search_capture`):** when the operator enables several backends via `MANTISFETCH_SEARCH_PROVIDERS` (e.g. `bocha,tavily`), pass `provider` to pick one per call. Recommended dual-region pattern: query a China engine with a Chinese query (`provider: "bocha"`, `lang: "zh"`) **and** a global engine with an English query (`provider: "tavily"`, `lang: "en"`) — call twice and keep the two result sets separate; MantisFetch does **not** merge them. Omit `provider` for the server default (primary + fallback chain). An unconfigured/unlisted provider name returns `400`.
