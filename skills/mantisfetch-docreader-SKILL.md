...
| `429 uploads being received or queued for parse hold N bytes` | Uploads still arriving plus those queued for parse already take the byte budget; this one was refused before any of it was read | Wait the seconds in `Retry-After`, then resend the whole request. Nothing was stored |
